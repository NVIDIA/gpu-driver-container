...
docker build -t "install-${CI_PROJECT_NAME}:${DRIVER_VERSION}" --build-arg DOCKER_VERSION=${COREOS_DOCKER_VERSION} --build-arg DRIVER_VERSION=${DRIVER_VERSION} .
