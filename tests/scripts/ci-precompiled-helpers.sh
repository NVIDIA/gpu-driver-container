get_kernel_versions_to_test() {
    if [[ "$#" -ne 5 ]]; then
	    echo " Error:$0 must be called with BASE_TARGET KERNEL_FLAVORS DRIVER_BRANCHES DIST LTS_KERNEL" >&2
	    exit 1
    fi
    ...
    for kernel_flavor in "${KERNEL_FLAVORS[@]}"; do
        for DRIVER_BRANCH in "${DRIVER_BRANCHES[@]}"; do
            source ./tests/scripts/findkernelversion.sh "$BASE_TARGET" "${kernel_flavor}" "$DRIVER_BRANCH" "$DIST" "$LTS_KERNEL" >&2
